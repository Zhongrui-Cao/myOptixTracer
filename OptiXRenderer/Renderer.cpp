...
void Renderer::initPrograms()
{
    ...
    programs["miss"]["backgroundColor"]->setFloat(0.0f, 0.0f, 0.0f);
...
std::vector<unsigned char> Renderer::getResult()
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            optix::float3 pixel = bufferData[(height - i - 1) * width + j];
...
void Renderer::buildScene()
{
    ...
    programs["rayGen"]["width"]->setFloat(width);
    programs["rayGen"]["height"]->setFloat(height);
